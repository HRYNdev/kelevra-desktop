...
//
// @param estSet — сеть физически есть (спрашивается у ОС уровнем выше, см.
// Sluzhitel.SprosFizicheskoySeti / FizicheskiySetevoyAdapterPodnyat, поэтому
// параметр, а не прямой вызов отсюда).
...
func (a *Avtorezhim) Zahod(ctx context.Context, estSet bool, dovereno bool) (nablyudeniye Nablyudeniye, izmenilos bool, tekushcheye Sostoyanie) {
	...
	// Подзонды идут один за другим на ОБЩЕМ ctx, и это намеренно: внешний
	// дедлайн остаётся жёстким потолком всего захода. Соблазн выдать
	// каждому подзонду свежий бюджет через context.WithoutCancel(ctx)
	// проверен и отвергнут 28.08: он лечит каскад, но заодно снимает
	// единственный предохранитель — зависший зонд после этого висит вечно,
	// и кнопка «Подключиться» не поднимает защиту НИКОГДА (стенд
	// TestPodklyuchitTaimautVsyoRavnoPodnimaetZashchitu в internal/sluzhba
	// виснет ровно на этом). Неизвестность обязана упираться в дедлайн и
	// трактоваться как «не дома» — это дороже лишнего VPN дома.
	//
	// Каскад лечится не здесь, а РАЗМЕРОМ бюджета у вызывающей стороны:
	// внешний таймаут обязан вмещать СУММУ номиналов подзондов
	...
	// следователя 28.08 поймал ровно это (DNS честно ответил за 280мс
	// вместо 50мс, тот же самый честный трафик получил другой вердикт
	// только из-за каскада бюджета, не из-за сети).
	if dedlayn, est := ctx.Deadline(); est {
		log.Printf("авторежим: DNS-подэтап занял %dмс, прямому зонду от общего бюджета захода осталось %s (если это меньше его номинала — вердикт «не дома» наш, а не сети)", dnsZanyalo.Milliseconds(), time.Until(dedlayn).Round(time.Millisecond))
	} else {
