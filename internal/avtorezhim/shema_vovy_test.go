...
// Схема, которую просил хозяин, проверенная СЦЕНАРИЯМИ, а не устройством кода.
// Его слова, 27.08: «жму подключить → сам определяет → дома "режим ожидания",
// вне дома включается». И четырежды повторённая жалоба (25.08, 28.08, 29.08,
// 30.08): «при переключении обратно на вайфай впн не выключился».
//
...
// TestDomaPriStarteZashchitaNePodnimaetsya — «дома "режим ожидания"» (хозяин,
// 27.08). Человек дома, защиты нет: авторежим обязан опознать дом и НЕ
...
// TestUshelIzDomaZashchitaPodnimaetsyaSama — «вне дома включается» (хозяин,
// 27.08). Человек был дома в режиме ожидания, ушёл, Windows прислала событие
...
func TestVernulsyaDomoyZashchitaOpuskaetsyaSama(t *testing.T) {
	...
	if st.podnyat() {
		t.Fatal("вернулись домой, а защита всё ещё поднята — жалоба хозяина 25/28/29/30.08 жива")
	}
