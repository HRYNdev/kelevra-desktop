...
func TestZadvizhkaShumOdinNablyudeniyeSbivaetSchyot(t *testing.T) {
	z := NovayaZadvizhka(VneDoma)
	z.Predlozhit(Doma)       // 1-е подтверждение кандидата Doma
	z.Predlozhit(Neizvestno) // шум — кандидат сбрасывается на Neizvestno
	if izm := z.Predlozhit(Doma); izm {
...
func TestZadvizhkaPodtverzhdeniyeTekushchegoNeTratitSchyotchik(t *testing.T) {
	z := NovayaZadvizhka(VneDoma)
	z.Predlozhit(Doma)    // 1-е подтверждение кандидата Doma
	z.Predlozhit(VneDoma) // подтвердили текущее — не должно засчитаться в пользу Doma
	if izm := z.Predlozhit(Doma); izm {
