...
func PostavitSHodom(ctx context.Context, klient *http.Client, n Novaya, putExe string, hod Hod) error {
	...
	// Обычно привычное имя <имя>.old свободно, и хвост зовётся как всегда.
	// Но 09.09.2026 на машине человека оно оказалось ЗАНЯТО, и это встало
	// поперёк всех будущих обновлений сразу. Разбор посекундно: копия,
	// поднятая в 10:11 (значок трея, ключ --trey), пережила обновление
	// 14:10 на 0.6.64 — и после той замены её собственный образ на диске
	// стал называться Kelevra.exe.old. Живой образ ни удалить, ни заменить
	// нельзя: os.Remove отказывает молча, а следующее переименование падает
	// с «Access is denied». В журнале это выглядело так:
	//
	//   19:24:45 не вышло (не могу отодвинуть текущее приложение:
	//            rename ...\Kelevra.exe ...\Kelevra.exe.old:
	//            Access is denied.), работаю старой версией
	//
	// Беда тут не в одной сорванной установке, а в том, что имя занято,
	// ПОКА ЖИВА та копия, — то есть обновления встают навсегда и сами уже
...
func hvosty(putExe string) []string {
	...
	// <имя>.old1, .old2 … — запасные имена, которые берёт Postavit, когда
	// привычное занято живой копией прежней версии (см. svobodnoyeImyaHvosta).
	// Без этой строки такой хвост не убрал бы никто и никогда: уборка ищет
	// ровно то, что перечислено здесь.
	if musor, err := filepath.Glob(putExe + ".old[0-9]*"); err == nil {
...
// svobodnoyeImyaHvosta — куда отодвинуть прежнюю сборку, когда привычное
// <имя>.old занято.
//
// Занято оно бывает ровно по одной причине: рядом живёт копия прошлой
// версии, чей образ и лежит под этим именем (разбор — в Postavit). Номер
// подбирается перебором, а не берётся из времени: имена остаются короткими
// и предсказуемыми, а уборка ловит их одним шаблоном .old[0-9]*.
//
// Сотня попыток — с огромным запасом: столько переживших копий на машине не
// бывает, а если случится немыслимое, метка времени всё равно даст свободное
// имя, и обновление не сорвётся из-за имени файла.
func svobodnoyeImyaHvosta(putExe string) string {
	for i := 1; i < 100; i++ {
		kandidat := fmt.Sprintf("%s.old%d", putExe, i)
		if _, err := os.Stat(kandidat); os.IsNotExist(err) {
			return kandidat
		}
	}
	return fmt.Sprintf("%s.old%d", putExe, time.Now().UnixNano())
}
