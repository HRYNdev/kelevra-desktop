...
type Nastroyki struct {
	...
	// решает клиент (хозяин, 27.08: «жму подключить → сам определяет → дома
	// "режим ожидания", вне дома включается»). Как только человек сказал
	// «нет» — клиент больше не решает за него ничего, пока тот сам не
	// вернёт тумблер.
	//
