...
func (s *Sluzhba) PrinyatZhivoeYadro(ctx context.Context, sledTunnelya string) {
	...
	default:
		...
		// И ровно тот же возврат связи, что в ветке «записки нет». Для
		// человека эти два случая неразличимы: он обновился и остался без
...
//
// Проверка живого ядра здесь не нужна, и зовут этот метод оба случая, где
// принимать нечего: записки нет вовсе — без неё живое ядро всё равно не взять;
// записка есть, но не опознана — такое ядро выше по PrinyatZhivoeYadro уже
// погашено.
