...
func TestOtklyuchitRukamiGasitAvtomatIBolsheNeReshaetSam(t *testing.T) {
	...
	if o.AvtorezhimPolozhenie != "автоматический режим выключен, переключение вручную" {
...
func TestPolozhenieAvtorezhima(t *testing.T) {
	...
	sluchai := []struct {
		...
		{"домашняя сеть, ядро стоит — подключение не требуется", true, false, doma, stoit, "", "домашняя сеть — подключение не требуется"},
		{"домашняя сеть, ядро ещё работает — переход виден", true, false, doma, rabotaet, "", "домашняя сеть — отключаюсь"},
		{"сеть не домашняя, ядро работает", true, false, vne, rabotaet, "", "сеть не домашняя — подключено"},
		{"сеть не домашняя, ядро ещё стоит — переход виден", true, false, vne, stoit, "", "сеть не домашняя — подключаюсь"},
		{"тип сети ещё не известен", true, false, "", stoit, "", "идёт определение типа сети"},
		{"длящаяся слепота названа человеку", true, false, doma, stoit, "физический сетевой адаптер не найден",
			"тип сети не определён: физический сетевой адаптер не найден"},
		{"человек решает сам", false, true, doma, rabotaet, "", "автоматический режим выключен, переключение вручную"},
		{"человек решает сам — даже при включённом тумблере это его слово", true, true, doma, rabotaet, "", "автоматический режим выключен, переключение вручную"},
