...
// Ступени «поднять совсем без правил» БОЛЬШЕ НЕТ — и это осознанная замена
// прежнего решения, а не недосмотр.
//
// Как было и почему так решили тогда: если и встроенный комплект не поднял
// ядро, лестница докатывалась до BezSetevyhPravil, «иначе человек вовсе без
// связи». Ценность понятная — хоть какая-то связь лучше никакой.
//
// Что показал живой замер 08.09. Без правил в туннель уходит ВЕСЬ трафик,
// включая российские сайты, и они отвечают человеку «выключите VPN». То есть
// связь есть, а пользоваться ей нельзя: в нашей стране такой режим не
// запасной вариант, а поломка. Слово Вовы в тот же день: «нахуй ваще такой
// режим, он не актуален в нашей стране».
//
// Новое правило: не поднимать связь и назвать причину. Цена названа вслух —
// при двойном отказе правил человек остаётся без связи до следующей попытки.
// Она принята сознательно: связь, которая ломает половину сайтов, хуже её
// отсутствия, а комплект правил теперь покрывает боевой профиль поимённо
// (internal/pravila), так что сюда попадают только при настоящей беде.
func TestBezPravilSvyazNePodnimaetsyaVovse(t *testing.T) {
	s := gotovStendLestnicy(t)

	popytok := 0
	s.zapustitYadro = func(ctx context.Context) error {
		popytok++
		// И свежие правила, и встроенный комплект отказали.
		return fmt.Errorf("%s", oshibkaIstochnikaPravil)
	}

	err := s.PodnyatZashchitu(context.Background())
	if err == nil {
		t.Fatal("правил нет ни свежих, ни встроенных, а служба отчиталась успехом — " +
			"значит связь подняли без разбора трафика, и весь он ушёл в туннель")
	}
	if popytok < 2 {
		t.Fatalf("ядро звали %d раз, ждали хотя бы 2: встроенный комплект обязан быть испробован", popytok)
	}
}
