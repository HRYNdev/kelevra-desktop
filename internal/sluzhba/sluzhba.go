...
// (требование 27.08, повторено 28.08 и 29.08), а заметен ровно в тот момент,
...
//
// Три секунды, и это потолок терпения, а не расчётное время: по замеру 09.09
// весь комплект (23 набора, ≈1 МБ) скачивается за 1-2 секунды по домашнему
// каналу. Дольше держать нажатую человеком кнопку нельзя — гейт на настоящей
// Windows поймал это сразу: пятнадцатисекундный срок сдвигал подъём защиты
// настолько, что авторежим успевал зайти второй раз и поднять её повторно.
//
// Главный запас не здесь: кеш наполняется заранее — при запуске службы и при
// каждом сохранении профиля (SveryatPravila зовёт сверку до первого тика).
// Этот шаг — последняя подстраховка для случая «профиль только что приехал, а
// человек уже жмёт кнопку».
var SrokPervogoNapolneniyaKesha = 3 * time.Second
