...
// Не поднялась и половина — приложение обязано убрать за собой ВСЁ.
//
// Беда с живой машины 08.09, чужая сеть: связь не поднялась ни полная, ни
// половинная, а запись системного прокси осталась висеть в реестре и
// указывала на ядро, которого нет. Браузер после этого мёртв целиком, при
// живой сети: само приложение ходит напрямую и прекрасно себя чувствует, а
// человек видит «интернета нет» и не понимает, при чём тут Kelevra.
//
// Проверяется СЛЕДСТВИЕ, а не устройство уборки: после полного провала на
// диске не остаётся следа туннеля, а ядро не числится работающим. Саму запись
// прокси на линуксе не проверить — proksi.Snyat там заглушка (см. её
// комментарий), и это честный предел проверки, а не недосмотр.
func TestNiPolnyyNiPolovinnyyUbiraetZaSoboy(t *testing.T) {
	s := stendSPravami(t)
	// След прошлого сеанса: уборка обязана снять его и на этом пути тоже.
	tunnel.Otmetit("tun125", 4242)
	snyatiy := 0
	...
	s.zapustitYadro = func(ctx context.Context) error {
		popytok++
		// Валятся обе попытки: и полный режим, и откат в режим браузеров.
		return fmt.Errorf("%s", oshibkaTunnelya)
	}

	err := s.PodnyatZashchitu(context.Background())
	if err == nil {
		t.Fatal("обе попытки провалились, а служба отчиталась успехом — окно покажет зелёный круг поверх пустоты")
	}
	if popytok < 2 {
		t.Fatalf("ядро звали %d раз — откат в режим браузеров даже не пробовали", popytok)
	}
	// Главное следствие: за неудачей не осталось следа. Пока эта уборка
	// стояла только в удачных ветках, человек получал сломанный браузер.
	if _, _, est := tunnel.ProchestMetku(); est {
		t.Fatal("след попытки остался на диске: следующий запуск будет искать адаптер, которого нет")
	}
	if s.Yadro != nil && s.Yadro.Sost() == yadro.Rabotaet {
		t.Fatal("ядро числится работающим после полного провала — за ним останется висеть системный прокси")
	}
	// ГЛАВНОЕ. Ровно эта строка отделяет «человек остался без обхода» от
	// «человек остался без интернета вовсе»: запись прокси указывает на
