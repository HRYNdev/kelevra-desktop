...
func Krutit(rabota func(ctx context.Context)) error {
	// Каждый старт подправляет свои же настройки восстановления: у тех, кому
	// служба поставлена старой версией, срок сброса счётчика отказов остался
	// суточным, и три обновления подряд оставляли службу лежать совсем
	// (замер 08.09, разбор в vinsluzhba_windows.go: Ustanovit).
	PopravitSamopodyom()
