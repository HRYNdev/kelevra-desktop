...
// ZagolovokPasporta — одним заголовком всё, чем устройство описывает СЕБЯ,
// кроме имени и версии: права, режим, возраст профиля, суммы файлов.
//
// Зачем одной строкой, а не десятком заголовков. Каждый заголовок едет в КАЖДОМ
// запросе к серверу, а их у клиента много; десяток полей раздул бы обмен на
// пустом месте. И расширять строку можно без правки сервера: он разбирает
// «ключ=значение» через точку с запятой и незнакомые ключи просто кладёт рядом.
//
// Зачем вообще. Разбор 10.09.2026: сервер знал про устройство пять полей и не
// мог ответить ни на «сколько живо копий», ни на «человек сам переключил режим»,
// ни на «какой возраст профиля» — а именно из этого складывается половина
// диагнозов, которые иначе стоят по полсессии каждый.
//
// Пустая строка — заголовка нет вовсе: старые копии клиента его не шлют, и
// сервер к отсутствию готов.
func ZagolovokPasporta(h http.Header, pasport string) {
	if p := vASCII(pasport); p != "" {
		h.Set("X-Pasport", p)
	}
}
