...
// достигнута, это не ошибка. Жалоба 23.08: ошибка при отключении, когда
// открыты 2 приложения.
//
// Эта половина теста бьёт ровно в развилку «ядро уже держим» и потому не
...
func TestZapustitNaZanyatomYadreNeOshibka(t *testing.T) {
	y := &Yadro{}
	// «ядро уже работает» — дальше развилки не идём. С 11.09.2026 процесс
	// живёт в держателе (см. derzhatel): ядро бывает своим и принятым, и
	// развилка смотрит на него, а не на *exec.Cmd напрямую.
	y.derzh = &derzhatel{pid: 4242, cmd: &exec.Cmd{}}
	defer func() { y.derzh = nil }()
