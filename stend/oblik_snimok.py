...
на живой машине человека — значит делать из него бета-тестера (требование 20.08).
...
ZHARGON = ["ядро", "ядра", "ядру", "ядром", "туннел", "прокси-режим",
           ...
           # (жалобы 22.08 и 23.08: непонятно, почему это вообще называется
           # защитой). Корень, а не целое слово: ловит «защита»,
...
# приняты глазами.
#
# Сейчас список ПУСТ, и это не забывчивость. Единственное исключение
# («туннель») жило тут ровно полдня — под подпись под кругом «дома интернет и
# так открыт, туннель выключен», взятую слово в слово с телефона. Вечером
# 02.09 эта подпись заменена («нажмите — проверю сеть, дома не включаю»: круг
# дома стал нажимаемым, и подпись обязана говорить про НАЖАТИЕ, а не про
# состояние), слова «туннель» в видимом тексте не осталось — ни в окне, ни в
# заметках konfig.go. Исключение, переживающее строку, ради которой оно
# заведено, — это дыра в гейте, открытая навсегда и незаметно.
ZHARGON_ISKLYUCHENIYA = []
...
BAZA = {"versiya": "0.5.3", "kod_est": True, "sost": "stoit",
        ...
        "imya": "Егор", "do_unix": 1789430400, "rezhim": "", "zametka": "",
        ...
        "kod_maska": "***Et",
        "chelovek_imya": "Егор", "ustroystvo_imya": "ASUS TUF Gaming B550-PLUS"}
...
SCENY = {
    ...
    # 02.09: та же ступень, но причина известна поимённо. Имя сетевого
    # адаптера осталось занято остатком прошлого подключения (замер с боевой
    # машины 01.09: «create adapter: файл уже существует | open existing
    # adapter: элемент не найден», ядро падало через 15 секунд). Приложение
    # берёт соседнее свободное имя само, и до этой сцены дело доходит только
    # когда заняты и соседние, — поэтому текст не шлёт нажимать ещё раз, а
    # называет причину (konfig.ZametkaAdapterZanyat).
    "25_adapter_zanyat": dict(BAZA, sost="rabotaet", pid="8124", rezhim="proksi",
                              vniz_bayt=18_874_368, vverh_bayt=1_572_864,
                              chastichnaya=True,
                              zametka=ZAMETKI["ZametkaAdapterZanyat"]),
    ...
    # Кто-то другой, с длинным именем, — и окно обязано остаться
    # читаемым: 420 px не растянешь.
    "16_dlinnoe_imya": dict(BAZA, sost="rabotaet", pid="8124", rezhim="tunnel",
                            imya="Константин Афанасьевич (семейный)",
                            zametka=ZAMETKI["ZametkaVes"]),
    ...
    # рамку/цвет (.uzel.vybran), и явную галочку ✓ (жалоба 29.08: в ручном
    # режиме выбор узла сделан криво — рамки одной было мало).
...
SCENY.update({
    ...
    "33_podpiska_odno_ustroystvo": dict(BAZA, chelovek_imya="Лена",
...
# Шторка «Подписка» (01.09 — вкладкой, по жалобе на отсутствие вкладки
# подписки; 02.09 — шторкой, открываемой карточкой #karta-podpiska: делать
# как на телефоне, а там вкладок две и подписка живёт карточкой на
...
class Ruchki(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.startswith("/api/obnovlenie"):
            ...
            # виснет на «Проверяем…» (жалоба 22.08, эталон — телефон).
...
ZHDEM_AVTOREZHIM = {
    ...
    # включил авторежим» — то же слово уже стоит в круге над ней (жалоба:
    # слово «защита» второй раз режет глаз; konfig.go: ZametkaVes). Проверяем ту
...
# --- 02.09: главный экран перестал влезать в окно ---------------------------
#
# Ещё один класс беды, мимо которого прошли ВСЕ щупы выше, и снова дошедший до
# глаз человека. Жалоба: главный экран снова приходится прокручивать, а
# масштаб на нём не сделан по-человечески.
#
# Что случилось. Строку подписки («Ключ доступа: ИМЯ · действует до 12
# сентября», 24px) заменила полноценная карточка #karta-podpiska в форме
# «Выхода» — 98px. Содержимое главного экрана выросло на 68px и перестало
# помещаться в 660: заметка внизу срезалась, до кнопки «Включить для всех
# программ» надо было крутить.
#
# Почему молчали остальные. Мёртвая высота меряет ПУСТОТУ под областью
# прокрутки — тут пустоты нет вовсе, наоборот, содержимого слишком много.
# Обрезка смотрит переполнение ВНУТРИ элемента с overflow:hidden — у #tab-set
# overflow:auto, то есть переполнение законно по определению и молчит.
# Геометрия меряет достижимость ПОСЛЕ scrollIntoView — прокрутив, дотянуться
# можно до всего, потому она и зелёная. Ни один из них не задаёт вопрос,
# который задал человек: «а надо ли вообще крутить?»
#
# Что мерим. Ровно это: scrollHeight против clientHeight у #tab-set. Порога
# нет и быть не может — «полоса прокрутки появилась или нет» решает сам
# браузер, и любое придуманное число тут было бы порогом наугад, ровно тем,
# от которого этот файл уже дважды отказывался. Один пиксель переполнения —
# это уже прокрутка, значит и беда.
#
# Запас на будущее меряется отдельно, глазами: после правки 02.09 у самой
# полной обычной сцены (4_rabotaet: круг, подсказка, две карточки, заметка,
# кнопка полной защиты) остаётся ~20px до края, так что дрожание отрисовки
# на пиксель этот щуп не разбудит.
PROKRUTKA_JS = """() => {
  const t = document.getElementById("tab-set");
  if (!t || !t.offsetParent) return [];   // экран кода / вкладка настроек
  if (t.scrollHeight <= t.clientHeight) return [];
  return [`главный экран не влез в окно: содержимое #tab-set ${t.scrollHeight}px ` +
          `при высоте ${t.clientHeight}px — лишние ` +
          `${t.scrollHeight - t.clientHeight}px это полоса прокрутки там, где её ` +
          `быть не должно: низ экрана срезан, и до последней кнопки надо крутить`];
}"""

# Сцены, где прокрутка ГЛАВНОГО экрана законна. Список явный и короткий, а не
# порог «сколько px простить»: простить N пикселей значит разрешить любой
# карточке подрасти на N, и следующая правка вёрстки въедет в этот зазор
# молча — ровно так и появилась беда 02.09.
#
# «5_slomalos» — единственная такая сцена, и была ею ещё ДО правки подписки
# (замер против версии a90dd86: все сцены влезали, эта переполнялась на 24px).
# Причина не в вёрстке: разбор беды «Windows не дал создать сетевой адаптер»
# — это заголовок + совет + ссылка на подробности, четыре строки текста,
# который приходит от ядра и длину которого вёрстка не выбирает. Резать его
# ради «влезло» значило бы прятать от человека ровно то, ради чего блок и
# заведён. Остальные три сцены беды (11/12/13) короче и влезают — их тут нет
# нарочно: они сторожат, что беда обычной длины экран не ломает.
PROKRUTKA_ZAKONNA = {"5_slomalos"}


def proverit_prokrutku(str_, imya_sceny):
    """Главный экран обязан влезать в 420x660 без прокрутки."""
    if imya_sceny in PROKRUTKA_ZAKONNA:
        return []
    return [f"{imya_sceny}: {b}" for b in str_.evaluate(PROKRUTKA_JS)]


def kontrol_prokrutki(br, port):
    """Щуп обязан покраснеть, если вернуть экрану вчерашнюю высоту.

    Порча — те самые меры, которыми экран ужат вечером 02.09, и в ту же
    сторону: метка карточек обратно на свой этаж, padding карточек 12→16,
    круг 196→210. Замер на сцене 4_rabotaet: содержимое #tab-set вырастает с
    484px до 555px при окне 504px — то же переполнение, что было видно на
    живой машине (там было 571 при 504).
    """
    sostoyanie["tek"] = SCENY["4_rabotaet"]
    str_ = br.new_page(viewport={"width": SHIRINA, "height": VYSOTA})
    str_.goto(f"http://127.0.0.1:{port}/index.html")
    str_.wait_for_timeout(700)
    # display:block нужен И на обёртке, И на самой метке: обёртка перестаёт
    # быть flex-строкой, но <span> внутри неё остались бы инлайновыми и легли
    # бы в ту же строку — порча вышла бы вдвое слабее настоящей беды.
    str_.add_style_tag(content=":root { --krug: 210px !important; } "
                               ".vyhod-verh { display: block !important; } "
                               ".vyhod-label { display: block !important; "
                               "margin-bottom: 6px !important; } "
                               ".karta-vyhod { padding: 16px !important; }")
    str_.wait_for_timeout(200)
    bedy = proverit_prokrutku(str_, "контроль-прокрутка")
    str_.close()
    return bedy
...
# --- 23.08: экран настроек был написан «по-свойски» (жалоба 22.08: надписи
# звучали как приятельская переписка, а не как настройки, и говорили не о
# том, что нужно видеть) — три голые кнопки без единого
...
# авторежима, 27.08 он переехал на главный экран полосой выбора режима
# (требование: управление режимом в одном месте, а не в двух), 28.08 вернулся
# на вкладку «Настройки» той же полосой (жалоба: настройки из отдельной
# вкладки переехали вниз главного экрана, и это неудобно), а 29.08 переехал
# ОКОНЧАТЕЛЬНО на вкладку «Сеть» — и рубрика «Сеть» на «Настройках»
# опустела, заголовок над пустотой убрали, а список ждал ровно двух
# заголовков.
#
# Рубрик снова ТРИ. Прямое расхождение с эталоном: на телефоне
# SimpleSettingsScreen.kt держит «Сеть» / «Приложение» / «Подписка», и в
# «Сети» лежит автозапуск («Подключение восстанавливается при включении
# телефона»). В окне автозапуск всё это время стоял в «Приложении» — рубрик
# было две, и не тех. Автозапуск (вместе с парной ему строкой службы Windows,
# которая про то же включение при входе в систему) вернулся в «Сеть», и
# заголовок вернулся вместе с ним — не пустой рубрикой, а над двумя живыми
# строками. Пустой она снова стать может (не-Windows сборка + уже
# поставленная служба) — тогда окно прячет и заголовок, и карточку разом,
# см. setPusta в index.html.
...
def proverit_zagolovki_blokov(str_, imya_sceny):
    ...
    (SimpleSettingsScreen.kt: три KGroupTitle в этом же порядке).

    Было «Приложение»/«Доступ»/«Если что-то не так» — последний и есть та
    самая «по-свойски» надпись (жалоба 22.08).
    """
...
def proverit_zhivaya_tochka(str_, imya_sceny, sost):
    ...
    Слово «защищено» из шапки убрано (жалоба 24.08: слово «защита» второй раз
    режет глаз), и единственным, что шапка ещё говорит, осталась цветная точка:
    ...
    with socketserver.TCPServer(("127.0.0.1", 0), Ruchki) as srv:
        ...
        with sync_playwright() as p:
            ...
            for imya, sost in SCENY.items():
                ...
                bedy += proverit_prokrutku(str_, imya)
            ...
            kontroli = {
                ...
                "прокрутки главного экрана": (kontrol_prokrutki(br, port),
                                              "вчерашняя высота карточек и круга (+68px) его не разбудила"),
