...
ZHARGON = ["ядро", "ядра", "ядру", "ядром", "туннел", "прокси-режим",
           ...
           # (Вова, 22.08: «включив полную защиту, почему она ваще нахуй
           # защита называется непонятно»; 23.08: «ПОЧЕМУ НАХУЙ ВСЕ ЕЩЕ КАКАЯ
           # ТО НАХУЙ ЗАЩИТА»). Корень, а не целое слово: ловит «защита»,
...
#
# 24.08: .lenta стала клипаться по var(--taby-vysota) вместо «bottom:0»
# (index.html) — список больше не рендерится ЗА панелью вовсе (диагноз по
# снимку 4_rabotaet.png: последний узел торчал из-под панели уже на первом
# кадре, без единой прокрутки). Из-за этого клипа старые 140px порчи уже не
# хватало: --taby-vysota(66) сам вычитается из бокса ленты ДО --nizhnyaya(84),
# и суммарный запас (66+84=150px) съедал порчу, щуп молчал (замерено: 140px
# и 150px — 0 бед, 160px — уже 1). Порчу подняли настолько, чтобы бить с
# запасом выше этой новой суммы, а не впритык к ней.
PORCHA_CSS = "#vkladki { height: 200px !important; }"
...
# 27.08: секция «Сеть» несла единственный пункт — тумблер авторежима, а он
# переехал на главный экран полосой выбора режима (Вова: «управление
# режимом должно быть в одном месте, а не в двух»). В настройках под «Сеть»
# больше ничего не осталось — секция убрана вместе с рядом, а не оставлена
# пустым заголовком.
