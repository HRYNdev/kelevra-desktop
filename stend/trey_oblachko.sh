...
# точное число.
schet_metki() {
  grep -cF "трей (не-Windows заглушка): подсказка значка -> \"Kelevra: вышла версия $2" "$1" 2>/dev/null || true
}
...
printf '\nВСЁ ЖИВЬЁМ: %d/%d сцен зелёных. Пузырь называет версию один раз на версию и переживает тики и перезапуск, а метка на значке переживает сам пузырь.\n' "$SCENA_N" "$VSEGO"
