...
Зачем он появился. Жалоба 30.08: полоса прокрутки в окне выглядит криво.
Снимок
...
def zapustit_xvfb():
    ...
    ВНЕ ЛИНУКСА ВИРТУАЛЬНЫЙ ДИСПЛЕЙ НЕ НУЖЕН И ЕГО НЕТ. На Windows (а это
    родная машина продукта) headed-браузер открывается на настоящем рабочем
    столе и полосу рисует сам, а `xdpyinfo` и `Xvfb` — программы X11, которых
    в системе нет вовсе. До этой правки щуп звал их безусловно и падал на
    первой же строке трассой FileNotFoundError [WinError 2], не дойдя ни до
    одного замера: на Windows он не был ни зелёным, ни красным — он не
    запускался. Проверяем НАЛИЧИЕ программ, а не имя системы: тот же ответ
    даёт и голый линукс-контейнер без X-утилит, где Xvfb всё равно не поднять,
    и там честнее пойти на существующий DISPLAY, чем упасть трассой."""
    if not shutil.which("xdpyinfo") or not shutil.which("Xvfb"):
        return None
...
def sdelat_porchennuyu_kopiyu(rabochaya_papka):
    ...
    беда, на которую пришла жалоба, воспроизведённая нарочно."""
