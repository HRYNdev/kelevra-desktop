...
Пороги (заданы человеком, не подкручены под результат):
  ...
  (з) КАРТОЧКА «ВЫХОД» + ШТОРКА ВЫБОРА (29.08, ответ на заказ хозяина 27.08:
      ...
      293-325 карточка, 374-443 шторка). 29.08 хозяин про сам список узлов,
...
def proverit_kartu_i_shtorku(str_, imya_sceny, avtorezhim_vklyuchen_ozhidaem):
    ...
    живут в шторке, открывающейся кликом по карточке — заказ хозяина 27.08:
    «должно быть тупо выбор авто режим который сам все определяет, или
