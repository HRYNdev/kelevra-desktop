...
# и выходит кодом 7 (см. stend/obshchee.sh) — это тот же код, которым стенды
# сообщают приёмке (stend/vse.sh) «прибор мёртв, продукт не проверялся», а не
# 0 или 1, чтобы не читаться ни зелёным, ни бедой продукта.
#
...
# STEND_PORCHA=dve_sluzhby — самопроверка стенда (08.09): под wine нет
# способа заставить настоящий двойной запуск залипнуть по требованию
# (нет рычага на гонку --smena, см. шапку файла), поэтому доказать, что
# гейты (a)/(d) КРАСНЕЮТ на настоящей порче, стенд сеет порчу сам —
# поднимает ВТОРУЮ настоящую службу отдельным unix-процессом (--sluzhba,
# слушает свой порт, пишет свою метку — internal/kopiya.Zanyat её не
# проверяет, второй запуск ничем не блокируется) и НЕ гасит её вручную по
# ходу сценария. Гасится она в конце по PID (kill "$PORCHA_PID"), не
# через `pkill -f "Kelevra.exe"` из соседней оболочки — тем самым нельзя
# случайно убить чужой процесс с тем же именем в подстроке cmdline.
#
# Отдельный WINEPREFIX (замерено 08.09): если пустить порчу в ТОМ ЖЕ
# WINEPREFIX, что и сценарий, она пишет в тот же kelevra.log и ту же
# метку (kopiya.Zanyat перезаписывает zapushcheno.json), и тогда её
# unix-процесс глохнет в первую же секунду сценария смены режима —
# похоже на конфликт вокруг общей метки/профиля, не на сам факт второй
# копии. Свой WINEPREFIX убирает эту путаницу: порча остаётся ЧИСТО
# вторым живым unix-процессом с ролью "служба" и ничем больше не делится
# со сценарием — то самое, что должны ловить гейты (a)/(d).
PORCHA_PID=""
if [ "${STEND_PORCHA:-}" = "dve_sluzhby" ]; then
  echo "── STEND_PORCHA=dve_sluzhby: сею вторую живую службу отдельным процессом (самопроверка гейта) ──"
  WINEPREFIX_PORCHA="$KORFN/.wine_porcha"
  mkdir -p "$WINEPREFIX_PORCHA"
  env WINEPREFIX="$WINEPREFIX_PORCHA" KELEVRA_BEZ_OBNOVLENIYA=1 timeout 30 "$WINE" "$STEND/Kelevra.exe" --sluzhba >"$STEND/porcha.log" 2>&1 &
  PORCHA_PID=$!
  sleep 2
  if kill -0 "$PORCHA_PID" 2>/dev/null; then
    echo "  вторая служба живая (pid $PORCHA_PID, свой WINEPREFIX), журнал: $STEND/porcha.log"
  else
    echo "  ⚠️ вторая служба не поднялась (см. $STEND/porcha.log) — порча не засеяна"
  fi
fi
...
# Правка 08.09: раньше этот гейт стоял ДО (a)/(d) и их код становился
# мёртвым на площадке без WebView2 (сюда попадает КАЖДЫЙ прогон здесь) —
# (a)/(d) физически не успевали выполниться. (b) этой болезнью не страдал
# (стоял раньше по файлу) и уже умел отдавать КРАСНЫЙ поверх площадки —
# теперь (a) и (d) считаются ДО этой проверки и подчиняются тому же
# правилу: площадка не гасит уже найденную настоящую беду, а только
# запрещает объявлять ЗЕЛЁНЫЙ вердикт, которого сама не может доказать.
if grep -q 'ОТКАЗ: нет компонента WebView2' "$ZHURNAL" 2>/dev/null; then
  ...
  echo "     диалоге (skazat -> MessageBoxW, нажать «ОК» под wine некому)."
  echo "── журнал (хвост) ──"
  tail -25 "$ZHURNAL" 2>/dev/null | sed 's/^/  /'
  [ -n "$PORCHA_PID" ] && kill "$PORCHA_PID" 2>/dev/null
  pkill -f "Kelevra.exe" 2>/dev/null
  if [ "$bed" -eq 1 ]; then
    echo "── итог: КРАСНЫЙ (по (a)/(b)/(d) — см. выше, не связано с площадкой) ──"
    exit 1
  fi
  echo "── итог: ПЛОЩАДКА — не зелёный и не красный, (a)/(b)/(d) не нашли беды, но и не доказаны на этой площадке ──"
  exit 7
fi
