...
KONTROL=""
case "${1:-}" in
  --kontrol=a|--kontrol=b|--kontrol=c|--kontrol=d|--kontrol=e) KONTROL=${1#--kontrol=} ;;
  "") ;;
  *) echo "аргумент не понят: $1 (жду --kontrol=a|b|c|d|e)" >&2; exit 2 ;;
esac
...
zapusti_sluzhbu() {
  ...
  # сразу при старте, а не через период (заказ: обновление приходит само, а
  # ставит его человек нажатием — значит находка обязана прийти
...
# самого HTTP-обработчика, а копия смены его тут же и подчищает — если
# опоздать с проверкой, .old исчезнет раньше, чем мы на него посмотрим. То,
# что он потом ОБЯЗАН исчезнуть сам, проверяется ниже отдельно.
...
# ХВОСТ УБИРАЕТСЯ САМ. Жалоба 02.09 начинается с того, что рядом с приложением
# «появляется .old, и всё»: отодвинутая старая сборка ложилась на видное место
# в ту самую папку, куда человек скачал Kelevra.exe, — и оставалась там.
# Уборка (UbratHvost) в коде была и до 02.09, но звучала ДО ожидания смены,
# когда старая копия ещё жива и держит свой файл: Windows не даёт удалить
# запущенный .exe, os.Remove тихо отказывает, и хвост доживал до следующего
# запуска руками. Теперь уборка идёт ВТОРЫМ заходом, уже после подтверждённой
# смерти старой копии (cmd/kelevra/main.go), — то есть сама, без участия
# человека, и ровно тогда, когда откатываться больше некуда и незачем.
#
# Ждём, а не смотрим одним взглядом: уборка стоит ПОСЛЕ подъёма новой службы,
# то есть уже после того, как метка копии (по которой мы сюда и дошли) успела
# записаться.
HVOST_UBRAN=1
for _ in $(seq 1 40); do
  if [ ! -e "$DOM_A/Kelevra.old" ]; then HVOST_UBRAN=0; break; fi
  sleep 0.25
done
if [ "$KONTROL" = "e" ]; then
  # порча: требуем (неверно), что хвост так и остался лежать на виду.
  if [ "$HVOST_UBRAN" -ne 0 ]; then
    past "ставится и меняется" "(порча e) требуем, что хвост .old ОСТАЛСЯ" "и вправду остался — порча не сработала, перепроверь сцену" ""
  else
    past "ставится и меняется" "(порча e) требуем, что хвост .old остался" "хвост всё-таки убран, как и должно — сцена поймала подмену" ""
  fi
fi
if [ "$HVOST_UBRAN" -ne 0 ]; then
  past "ставится и меняется" "хвост .old убран САМ после подъёма новой копии" \
    "файл $DOM_A/Kelevra.old лежит на виду спустя 10с — человек увидит его рядом с приложением и решит, что обновление сломалось" \
    "$(ls -la "$DOM_A")"
fi
shag "ставится и меняется" "postavit→200 (версия $VERSIYA_OTVETA), файл+.old на месте, старая копия умерла, новая отвечает версией $NOVAYA_ZHIVA_VERSIYA, хвост .old убран сам" "тычок = поставилось, перезапустилось и прибрало за собой"
...
printf '\nВСЁ ЖИВЬЁМ: %d/%d сцен зелёных. Тычок ставит найденное обновление, уходит на смену и убирает за собой хвост .old — человеку остаётся только работающая новая версия.\n' "$SCENA_N" "$VSEGO"
