...
# Локаль wine — не косметика. Windows-путь wine переводит в путь unix через
# локаль процесса; при LANG=C (как в кроне и в моём заходе) любое имя файла с
# кириллицей просто НЕ СОЗДАЁТСЯ: «File not found». Замерено 26.08 на голом
# коде: под C.UTF-8 mkdir «без_прав» проходит, под C/POSIX и под несобранной
# en_US.UTF-8 — падает; нативно на линуксе проходит всегда. Продукт русский,
# имена профилей и заметок кириллические, так что без этой строки стенд слеп
# ровно к тем путям, по которым ходит человек.
