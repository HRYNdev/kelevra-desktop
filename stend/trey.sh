...
#      номер хендла не ноль — «значок: собран из znachok.ico ... HICON=0x…»;
...
stroka_znachok=$(printf '%s\n' "$zhurnal1" | grep -oE "значок: собран из znachok\.ico.*HICON=0x[0-9a-f]+")
...
if ! sobrat 2>&1; then
  ...
  otkaz=$(printf '%s\n' "$zhurnal2" | grep -c "значок: не удалось собрать значок из znachok\.ico.*беру системный значок")
  uspeh_na_lomanom=$(printf '%s\n' "$zhurnal2" | grep -c "значок: собран из znachok\.ico")
