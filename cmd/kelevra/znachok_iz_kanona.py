...
Геометрия (viewport 108x108, как в векторе):
  - фон:                 ПРОЗРАЧНЫЙ вне круга; правка 30.08 (хозяин: «иконку
                          нормально круглой сделать, че это за квадрат»)
                          убрала сплошной КВАДРАТ #111310. Но 30.08 вечером
                          хозяин: «иконка стала слишком серая» — без тёмной
  ...
  - кольцо:               центр (54,54), радиус 27, strokeWidth 4 (номинал;
                           на мелких .ico-размерах толщина увеличивается —
                           см. HINT_MIN_STROKE_PX ниже), обводка линейным
                           градиентом (27,27)->(81,81): #A8CC6B -> #7F9E4A,
                           заливка прозрачная
...
Хинтинг толщины кольца на мелких .ico (правка 30.08, вторая):
  Кадр 72 и strokeWidth 4 не меняются (канон), но при сжатии до sz<=~32
  линия физически тоньше 1.5px и рассыпается в антиалиасинг. Поэтому
  strokeWidth для КОНКРЕТНОГО итогового размера sz считается как
  max(RING_W, HINT_MIN_STROKE_PX * FRAME / sz) — то есть на 16x16 кольцо
  рисуется толще (в исходных unit'ах), чтобы после уменьшения линия давала
  не меньше 1.5 физических пикселей. На 32x32 и крупнее формула уже не
  меняет ничего (4*sz/72 там и так >=1.5px) — крупные размеры визуально
  не тронуты. Каждый размер поэтому рендерится ОТДЕЛЬНЫМ проходом
  draw_canvas(), а не общим кропом на одно и то же изображение.
...
def draw_canvas(s, ring_w=RING_W, flat=False):
    ...
    # --- кольцо: заливка прозрачная, только обводка width=ring_w градиентом
...
def render_size(sz):
    ...
    return cropped.resize((sz, sz), Image.LANCZOS)
