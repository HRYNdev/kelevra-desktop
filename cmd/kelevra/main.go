...
func main() {
	...
	// --tiho — для автозапуска с Windows и для смены после автообновления
	// (cmd/kelevra/obnovlenie.go: zapustitSmenuPosleObnovleniya шлёт ОБА флага
	// разом): поднять службу молча, без окна — там окно никто не просил.
	//
	// Беда 27.08, жалоба хозяина «оно тупо не работает пока не перезапустишь»:
	// раньше здесь стояло `tiho := estArg(argTiho) || estSmena` — то есть
	// ЛЮБАЯ смена (в том числе смена ПРАВ после согласия на UAC, которая
	// приходит БЕЗ --tiho, только с --smena) считалась тихой, и pokazatOkno
	// не звался никогда. Человек жал «Включить для всех программ», старое
	// окно закрывал свой же сторож (storozh_okna.go, как только его служба
	// переставала отвечать), а новое, уже повышенное окно просто не
	// появлялось — снаружи это выглядело как «нажал и ничего не произошло».
	// --smena — это сообщение О ПРИЧИНЕ смены (см. argSmenaPID), а не о том,
	// нужно ли окно; тихость смены целиком несёт свой собственный флаг.
	tiho := estArg(argTiho)
