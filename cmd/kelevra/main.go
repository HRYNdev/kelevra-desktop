...
func main() {
	...
	if pokazatLiOkno(tiho, smenaPID, priStarte, itogOkna) {
		...
		// Переезд службы на другой порт окно переживает САМО, не закрываясь:
		// сторож переводит его на новый адрес (storozh_okna.go). Открывать
		// второе окно в этом же процессе нельзя — go-webview2 падает внутри
		// себя, проверено на живом стенде 09.09.
		pokazatOkno(adres)
	}
}
...
func adresKopii(papka, putZhurnala string, smenaPID int) (string, bool, itogSmenyOkna) {
	...
	if smenaPID > 0 {
		...
		zhdatSmenu(smenaPID)
		// МЕТКУ СТИРАЕМ, ТОЛЬКО ЕСЛИ ОНА ОТ ТОЙ САМОЙ УМЕРШЕЙ КОПИИ.
		//
		// Разбор с живой машины 09.09, обновление 0.6.62 → 0.6.63. В 23:20:54
		// диспетчер поднял службу новой версии, она записала свою метку и
		// начала работать. Секундой позже стартовала эта ветка, стёрла метку
		// как «оставшуюся от умирающей», и стала ждать, когда служба поставит
		// новую. Служба второй раз её не ставит — ей незачем. Через двадцать
		// секунд копия решала, что службы нет, и поднимала свою: на машине
		// оказывалось два хозяина одного туннеля. Каждый следующий запуск
		// приложения добавлял ещё одного — у человека их стало три, они
		// дрались за сетевой адаптер и за файл кеша ядра, ядро падало, а окна
		// показывали каждое своё состояние.
		if _, pid, est := kopiya.KtoZanyal(papka); est && pid != smenaPID {
			log.Printf("место уже занял другой процесс (pid %d, а сменяем %d) — метку не трогаю", pid, smenaPID)
		} else {
...
// sledTunnelyaPriStarte — какой адаптер держала прошлая копия, если её след
// нашёлся на старте. Пусто, если следа не было.
//
// Зачем в пакетной переменной. Уборка следа стоит в самом начале main, ДО
// службы и до всего остального (см. её комментарий — иначе служба Windows до
// неё не доходит вовсе). Значит к моменту, когда служба спрашивает «держал ли
// кто-то туннель до меня», следа на диске уже нет: его сняла эта же уборка.
// Замер на стенде 11.09.2026: связь после обновления со старой версии не
// поднималась именно поэтому — проверка читала файл, стёртый минутой раньше
// в том же процессе.
var sledTunnelyaPriStarte string
...
func uborkaSledaTunnelya(papka string, zhiv tunnel.Adapter) {
	...
	// Запоминаем ДО всех выходов ниже: след снят, и другого способа узнать,
	// что прошлая копия держала туннель, у службы уже не будет.
	sledTunnelyaPriStarte = itog.Adapter
...
func rabotaSluzhby(vneshniy context.Context, papka, putZhurnala string, sTreem bool) {
	...
	s.PrinyatZhivoeYadro(ctx, sledTunnelyaPriStarte)
	...
	// Раз в полчаса копия спрашивает сервер, не поменялись ли списки правил.
	// До 09.09 этого не спрашивал никто: обновление кеша звалось ровно один
	// раз — сразу после удачного подъёма связи. У того, чья машина работает
	// сутками, правила не менялись никогда, и это была честная жалоба.
	go s.SveryatPravila(ctx, sluzhba.ShagSverkiPravil)
