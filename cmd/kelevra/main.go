...
func main() {
	...
	// Заказ Вовы 29.08: «чтоб это подтверждение тупо вылазило при старте
...
// на обычном старте (заказ Вовы 29.08: «чтоб это подтверждение тупо вылазило
...
//
//	25.08 (7f31c1c): жалоба «2 нахуй открыто» — при смене режима на секунды
//	      показывались два окна разом. Закрыл её тем, что новая копия стала
//	      молчать ВСЕГДА: `tiho := estArg(argTiho) || estSmena`.
//	27.08: жалоба «оно тупо не работает пока не перезапустишь» — то самое
